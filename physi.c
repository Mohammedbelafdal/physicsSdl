...
Entity newEntity(char *name, charge charge, SDL_bool isVisible, double positionx, double positiony, SDL_Texture *texture, variables *internalVariables)
{
    Entity Instance;
    Instance.name = name;
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            case SDL_BUTTON_LEFT:
                Entity particle = newEntity("particle", negative, SDL_TRUE, x, y, negativeTexture, internalVariables);
                entities[internalVariables->numberOfEntities - 1] = particle;
                printf("created %d\n particle", entities[internalVariables->numberOfEntities].charge);
                break;
            case SDL_BUTTON_RIGHT:
                particle = newEntity("particle", positive, SDL_TRUE, x, y, positiveTexture, internalVariables);
                entities[internalVariables->numberOfEntities - 1] = particle;
                printf("created %d\n particle", entities[internalVariables->numberOfEntities].charge);
                break;
            };
...

{
    ...
    variables internalVariables = {.numberOfEntities = 0, .enablePhysics = SDL_TRUE,.positiveCharge=1.3,.negativeCharge=-1};
    while (!close)
    {
